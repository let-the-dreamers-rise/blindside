...
type Timing = { readonly label: string; readonly seconds: number };
...
const timed = async <T>(label: string, run: () => Promise<T>): Promise<T> => {
  ...
  console.log(`  ${label.padEnd(28)} ${seconds.toFixed(1)}s`);
  return result;
};
...
const main = async (): Promise<void> => {
  ...
  if (state === null) {
    throw new Error("contract state disappeared");
  }
  const finalLedger = readLedger(state.data);
...
`);
  ...
  const evidence = {
    ...
    timings: timings.map((t) => ({ ...t, seconds: Number(t.seconds.toFixed(2)) })),
